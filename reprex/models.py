...
class neuromarkICNet(nn.Module):
    def __init__(self, num_classes=2):
        super(neuromarkNet, self).__init__()
        self.features = nn.Sequential(
            nn.Conv3d(53, 64*53, kernel_size=3,
                      stride=1, padding=1, groups=53),
            nn.BatchNorm3d(64*53),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=3, stride=3),

            nn.Conv3d(64*53, 128*53, kernel_size=3,
                      stride=1, padding=1),
            nn.BatchNorm3d(128*53),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=3, stride=3),

            nn.Conv3d(128*53, 192*53,
                      kernel_size=3, padding=1),
            nn.BatchNorm3d(192*53),
            nn.ReLU(inplace=True),

            nn.Conv3d(192*53, 192*53,
                      kernel_size=3, padding=1),
            nn.BatchNorm3d(192*53),
            nn.ReLU(inplace=True),

            nn.Conv3d(192*53, 128*53,
                      kernel_size=3, padding=1),
            nn.BatchNorm3d(128*53),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool3d([1, 1, 1]),
...
class neuromarkNetRegression(nn.Module):
    def __init__(self, num_classes=1):
        super(neuromarkNetRegression, self).__init__()
        self.features = nn.Sequential(
            nn.Conv3d(1, 64, kernel_size=3,
                      stride=1, padding=1),
            nn.BatchNorm3d(64),
